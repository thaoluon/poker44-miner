"""Poker44 miner backed by a trained LightGBM chunk-level detection model."""
...
class Miner(BaseMinerNeuron):
    ...
    Scores each chunk with a LightGBM classifier trained on the public
    Poker44 training benchmark (chunk-level behavioral + cross-hand
    consistency features). Falls back to the reference heuristic if the
    ...
    def __init__(self, config=None):
        ...
        self.model_manifest = build_local_model_manifest(
            ...
            defaults={
                ...
                "notes": (
                    "LightGBM classifier over chunk-level behavioral and "
                    "cross-hand consistency features, calibrated so the 0.5 "
    ...
    async def forward(self, synapse: DetectionSynapse) -> DetectionSynapse:
        ...
        bt.logging.info(
            ...
            f"({'lgbm model' if self.detection_model.ready else 'heuristic fallback'})."
        )
        return synapse
